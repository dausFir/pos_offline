    ...
    <div className="app-shell">
      ...
      <aside className="sidebar">
        <div className="sidebar-brand">
          ...
          <div>
            <div className="brand-name font-headline">RapiPos</div>
            <div className="brand-sub">Offline first</div>
          </div>
        </div>
          ...
          <aside className="drawer" onClick={e => e.stopPropagation()}>
            <div className="drawer-head">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span className="brand-name font-headline" style={{ fontSize: 14 }}>RapiPos</span>
              </div>
      ...
      <main className="main-content"><LicenseBanner /><Outlet /></main>

      <style>{`
        .app-shell { display: flex; min-height: 100dvh; background: var(--surface); }
        .sidebar { width: 248px; background: var(--surface-container-lowest); border-right: 1px solid var(--outline-variant); display: flex; flex-direction: column; padding: 0; position: fixed; top: 0; left: 0; bottom: 0; z-index: 30; }
        .sidebar-brand { display: flex; align-items: center; gap: 10px; padding: 23px 20px 17px; border-bottom: 1px solid var(--outline-variant); margin-bottom: 10px; }
        .brand-icon { width: 38px; height: 38px; background: var(--primary); border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 5px 14px rgba(8,118,106,.22); }
        .brand-name { font-weight: 800; font-size: 17px; color: var(--on-surface); line-height: 1; letter-spacing: -.035em; }
        .brand-sub  { font-size: 9.5px; color: var(--outline); text-transform: uppercase; letter-spacing: .12em; margin-top: 4px; font-weight: 700; }
        .sidebar-nav { flex: 1; padding: 0 12px; display: flex; flex-direction: column; overflow-y: auto; }
        .nav-item { display: flex; align-items: center; gap: 10px; padding: 9px 11px; border-radius: 10px; color: var(--on-surface-variant); text-decoration: none; font-size: 13px; font-weight: 600; transition: all .12s ease; border: none; background: none; cursor: pointer; width: 100%; white-space: nowrap; }
        .nav-label { }
        .nav-item:hover { background: var(--surface-container-low); color: var(--primary); }
        .nav-item--active { background: var(--primary-fixed); color: var(--primary); font-weight: 700; }
        .main-content { flex: 1; margin-left: 248px; min-height: 100dvh; background: var(--surface); }
        .mobile-topbar { display: none; position: fixed; top: 0; left: 0; right: 0; height: 58px; background: color-mix(in srgb, var(--surface-container-lowest) 92%, transparent); backdrop-filter: blur(12px); border-bottom: 1px solid var(--outline-variant); padding: 0 14px; align-items: center; justify-content: space-between; z-index: 25; }
        .drawer-scrim { position: fixed; inset: 0; background: rgba(24,28,32,.4); backdrop-filter: blur(3px); z-index: 40; display: flex; }
        .drawer { width: 286px; height: 100%; background: var(--surface-container-lowest); border-right: 1px solid var(--outline-variant); display: flex; flex-direction: column; box-shadow: var(--shadow-3); animation: slideInLeft .2s cubic-bezier(.34,1.3,.64,1); }
        ...
        @media (max-width: 768px) { .sidebar { display: none; } .mobile-topbar { display: flex; } .main-content { margin-left: 0; padding-top: 58px; } }
      `}</style>
    </div>
  );
}
