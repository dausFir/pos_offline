...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	if !validRoles[req.Role] {
		writeJSON(w, http.StatusBadRequest, models.APIResponse{Success:false, Error:"Role tidak valid"}); return
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(req.Password), bcrypt.DefaultCost)
	if err != nil { writeJSON(w, http.StatusInternalServerError, models.APIResponse{Success:false, Error:"Gagal hash password"}); return }
	...
	if err != nil { writeJSON(w, http.StatusConflict, models.APIResponse{Success:false, Error:"Username sudah digunakan"}); return }
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeJSON(w, http.StatusBadRequest, models.APIResponse{Success:false, Error:"ID tidak valid"}); return }
	if targetID == claims.UserID {
		writeJSON(w, http.StatusBadRequest, models.APIResponse{Success:false, Error:"Tidak bisa hapus akun sendiri"}); return
	}
	...
	if role == "super_admin" {
		writeJSON(w, http.StatusForbidden, models.APIResponse{Success:false, Error:"Tidak bisa menghapus akun super_admin"}); return
	}
	now := time.Now()
	database.DB.Exec(
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := database.DB.QueryRow("SELECT role FROM users WHERE id=? AND is_deleted=0", targetID).Scan(&existingRole); err != nil {
		writeJSON(w, http.StatusNotFound, models.APIResponse{Success: false, Error: "Pengguna tidak ditemukan"}); return
	}
